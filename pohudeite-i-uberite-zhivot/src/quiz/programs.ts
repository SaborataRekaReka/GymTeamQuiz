...
function buildRelevanceTags(
  ...
  const firstZone = matchedZones[0]
  if (firstZone) dynamic.push(firstZone.toLowerCase())
  const firstGoal = matchedGoals[0]
  if (firstGoal) dynamic.push(pickGoalTag(firstGoal))
