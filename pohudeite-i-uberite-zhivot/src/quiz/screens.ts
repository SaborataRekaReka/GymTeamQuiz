...
import { quizScreenMap as rawMap } from '../../data/quizScreenMap'
import type {
  InputScreen,
  LeadGateScreen,
  LoaderScreen,
...
function resolveIcons(icons: string[] | undefined): string[] | undefined {
  return icons ? icons.map((icon) => assetUrl(icon)) : icons
}
  ...
  switch (raw.type) {
    case 'single_choice': {
      const screen: SingleChoiceScreen = {
        ...
        options: raw.options ?? [],
        offer: raw.offer,
        optionIcons: resolveIcons(raw.option_icons),
      }
      return screen
    }
    case 'multi_choice': {
