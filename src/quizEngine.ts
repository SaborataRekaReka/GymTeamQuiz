...
export function moveBack(config: QuizFlowConfig, state: QuizRuntimeState): NavigationResult {
  let previousIndex = Math.max(state.currentScreenIndex - 1, 0)

  // Do not return users to the plan loader after it has already been shown.
  const previousScreen = config.screens[previousIndex]
  if (previousScreen?.id === 'plan_loader' && previousIndex > 0) {
    previousIndex -= 1
  }
