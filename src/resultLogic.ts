...
export function recommendPrograms(input: ResultCalculationInput): string[] {
  ...
  if (input.zones?.some((zone) => zone.includes('Живот'))) programs.add('Живот и талия')
  if (input.zones?.some((zone) => zone.includes('Ягодицы') || zone.includes('Бёдра'))) programs.add('Ягодицы и ноги')
  if (input.zones?.some((zone) => zone.includes('Спина'))) programs.add('Осанка и спина')
  if (input.trainingPlace === 'Дома') programs.add('Домашние тренировки')
  if (input.trainingPlace === 'В зале') programs.add('Тренировки для зала')
